...
export class CombatSystem {
   ...
   */
  // Around line 900 in CombatSystem.js
  createExplosionEffect(position, radius, color = 0xff0000) {
    try {
      ...
      // Use fewer particles based on radius (was using too many)
      const particleCount = Math.min(10, Math.floor(radius * 10));

      // Create sprite material for all particles (reuse material)
      ...
      // Create sprites instead of geometry-based particles
      const particles = [];
      ...
      // Use simpler animation approach with fewer operations
      let elapsed = 0;
      const animateDuration = 1.0; // 1 second total

      const updateExplosion = () => {
        // Single update for all particles
        ...
        particles.forEach(particle => {
          ...
          // Update age
          particle.userData.age += 0.016; // Assume 60fps
        ...
        if (stillActive && elapsed < animateDuration) {
          elapsed += 0.016;
